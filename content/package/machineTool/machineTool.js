          ...
          case'video':
            {
              ...
              for await(const value of (function*(){
                for(let key=1,length=11;key<length;key++){
                  window.console.log('++++ server100, video, origin, page:',key);
                  yield machineTool.loop(async()=>{
                    // await machineTool.fetch(`${URI}?ac=videolist&pg=${key}&wd=${body.data.data}`,'GET',undefined,undefined,undefined,'json',(data)=>{
                    //   return new window.Promise((resolve)=>{
                    //     if(!data.status){
                    //       length=0;
                    //       window.console.log('++++ server100, video, origin, error');
                    //       resolve('error');
                    //     }else{
                    //       if(data.result.list[0]){
                    //         machineTool.for(data.result.list,(...data)=>{
                    //           window.console.log(data);
                    //           if(data[2].type_id!==36&&data[2].vod_play_url){
                    //             result.push({
                    //               url:data[2].vod_play_url.match(/http[^#$]+m3u8/ig),
                    //               name:data[2].vod_name,
                    //               poster:data[2].vod_pic,
                    //               year:data[2].vod_year
                    //             });
                    //           }
                    //         },0);
                    //         resolve();
                    //       }else{
                    //         window.console.log('++++ server100, video_origin, page:',key,'no data');
                    //         resolve('break');
                    //       }
                    //     }
                    //   });
                    // });
                    await machineTool.fetch(`${URI}?pg=${search.page}&wd=${search.search}`,'GET',undefined,undefined,undefined,'text',(data)=>{
                      return new window.Promise((resolve)=>{
                        window.console.log(data.status);
                        if(!data.status){
                          length=0;
                          window.console.log('++++ server100, video, origin, error');
                          context.response.body=window.JSON.stringify('error');
                        }else{
                          const parser=new Parser();
                          const root=parser.parse(data.result);
                          window.console.log(root);
                          root.find(['rss','list','video']).forEach((node)=>{
                            result.push({
                              // url:data[2].vod_play_url.match(/http[^#$]+m3u8/ig),
                              name:node.getChild('name')?.getValue(''),
                              // poster:,
                              year:node.getChild('last')?.getValue('')
                            });
                            resolve();
                            window.console.log(node.getChild('name')?.getValue(''),node.getChild('last')?.getValue(''));
                          });
                        }
                      });
                    });
